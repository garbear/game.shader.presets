...
{
   struct rarch_video_shader_parameter parameters[GFX_MAX_PARAMETERS]; /* int alignment */
   /* If < 0, no feedback pass is used. Otherwise,
    * the FBO after pass #N is passed a texture to next frame. */
   int feedback_pass;
   int history_size;

   struct rarch_video_shader_pass pass[GFX_MAX_SHADERS]; /* unsigned alignment */
   struct rarch_video_shader_lut lut[GFX_MAX_TEXTURES];  /* unsigned alignment */
   unsigned passes;
   unsigned luts;
 ...
 **/
bool video_shader_load_current_parameter_values(config_file_t *conf, struct rarch_video_shader *shader);
 ...
 * Returns: true (1) if successful, otherwise false (0).
 **/
bool video_shader_load_preset_into_shader(const char *path, struct rarch_video_shader *shader);

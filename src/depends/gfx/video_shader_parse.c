 ...
 *
 * s is filled with the absolute path
 **/
 ...
 *
 **/
void video_shader_resolve_parameters(struct rarch_video_shader *shader)
{
   size_t i;
...
bool video_shader_load_current_parameter_values(
      config_file_t *conf, struct rarch_video_shader *shader)
{
   size_t i;
 ...
 **/
static bool video_shader_write_root_preset(const struct rarch_video_shader *shader,
      const char *path)
{
   size_t i;
   char key[64];
...
static bool video_shader_write_referenced_preset(
      const struct rarch_video_shader *shader,
      const char *path_to_save)
{
   ...
   struct rarch_video_shader *ref_shader        = (struct rarch_video_shader*)
   ...
   */
   ...
   else
   {
      ...
      {
         const struct rarch_video_shader_pass *pass      = &shader->pass[i];
         const struct rarch_video_shader_pass *root_pass = &ref_shader->pass[i];
      ...
      struct rarch_video_shader *shader)
{
   ...
   if (shader->num_parameters)
   {
      ...
      for (i = 0; i < shader->num_parameters; i++)
      {
         ...
         {
            struct rarch_video_shader_parameter *parameter =
...
static bool video_shader_combine_shaders(
      struct rarch_video_shader *combined_shader,
      struct rarch_video_shader *first_shader,
      struct rarch_video_shader *second_shader)
{
   size_t i, j;
      ...
      bool pressed_prev)
{
       ...
       && !string_is_empty(last_shader_preset_file_name))
   {
      ...
      {
         struct rarch_video_shader *shader = menu_shader_get();
...
bool video_shader_combine_preset_and_apply(
      ...
      struct rarch_video_shader *menu_shader,
      ...
      bool message)
{
   ...
   struct rarch_video_shader *shader_to_append = (struct rarch_video_shader*) calloc(1, sizeof(*shader_to_append));
   struct rarch_video_shader *combined_shader  = (struct rarch_video_shader*) calloc(1, sizeof(*combined_shader));
...
#ifdef HAVE_MENU
   struct rarch_video_shader *menu_shdr  = menu_shader_get();
